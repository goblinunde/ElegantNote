...
\documentclass[cn,11pt,normal]{elegantnote}
...
\begin{definition}
	设有一族集合$\set{A_{\alpha }:\alpha \in{\Lambda }}$，其中$\alpha $是在固定指标集$\Lambda$中变化的指标；则由一切$A_{\alpha }(\alpha\in\Lambda )$的所有元素组成的集合称为这族集合的\jiachu{并集}或\jiachu{和集}，记为$\newcup{\alpha \in \Lambda}{} A_{\alpha}$，它可表示为$$\bigcup_{\alpha \in \Lambda }A_{\alpha}=\set{x:\text{存在某个}\alpha \in\Lambda ,\text{使} x\in A_{\alpha}}$$
\end{definition}
